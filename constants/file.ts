...
export const addChat = (uid:string,msg:ChatMessage|null):boolean=>{
    ...
    stor.set(uid,JSON.stringify([msg??{msg:"INIT",who:'mid',time:Date.now()}]));
    return true;
};
...
export const rmChat = (uid:string):void=>{
    stor.delete(uid)
    readChat(uid).map(msg=>msg.uri&&RNFS.unlink(msg.uri.replace('file://', '')).catch(_e=>{}))
}
export const splitSend = async (file: FileInfo, send: SendChunk): Promise<boolean> => {
    const { uri, name,} = file;
    try {
        const exist = await RNFS.exists(uri.replace('file://',''));
        if (!exist) throw new Error('File not found');
        const bin = await RNFS.readFile(uri.replace('file://',''),'base64');
